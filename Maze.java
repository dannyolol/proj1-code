...
public class Maze {
    ...
    /**
     * Helper methods to set a cell as the start point of the maze. Calls setState with 'S' character.
     * @param x
     * @param y
    ...
    /**
     * Helper methods to set a cell as the end point of the maze. Calls setState with 'E' character.
     * @param x
     * @param y
     * @return  true if successful, false if out of bounds
     */
    ...
    /**
     * Helper methods to set a cell as empty (traversable). Calls setState with ' ' character.
     * @param x
     * @param y
     * @return true if successful, false if out of bounds
     */
     ...
     * The text file should contain the maze in the same format as the toString output (borderless), with walls represented by '#', empty spaces by ' ', the start point by 'S', and the end point by 'E'. 
     * The method reads the file line by line, updates the maze dimensions based on the imported maze, and fills the mazeStates array accordingly.
    ...
    /**
     * Increment the mark on a cell in the maze. If the cell is empty, it becomes marked with a single dot ('.'). If it's already marked with a single dot, it becomes marked with a colon (':'). 
     * If it's already marked with a colon, it becomes marked with a dash ('-'). If the cell is not empty or already marked once, the method prints an error message and returns false.
     * @param x
     * @param y
     * @return true if the mark was successfully incremented, false if the cell is not empty or already marked once, or if out of bounds
     */
    private boolean incMark(int x, int y) {
        ...
        System.out.println("Attempted to increment mark on cell (" + x + ", " + y + ") which is not empty or already marked once.");
        return false;
    }
    /**
     * Helper method to move the cursor in a given direction. The direction is represented as an integer (0 for up, 1 for right, 2 for down, 3 for left).
     * @param cursor
     * @param dir direction to move the cursor (0 for up, 1 for right, 2 for down, 3 for left)
     * @return an array containing the new coordinates of the cursor after moving in the given direction, or the original cursor if the move is invalid (e.g., out of bounds or into a wall)
     */
    private int [] moveCursor(int [] cursor, int dir) {
        ...
        int newY = cursor[1];
        switch (dir) {
            case 0: newX--; break; // Move up
            case 1: newY++; break; // Move right

            case 2: newX++; break; // Move down
            case 3: newY--; break; // Move left
            default: System.out.println("Invalid direction: " + dir + ". Use 0 for up, 1 for right, 2 for down, 3 for left."); return cursor;
        }
        ...
        return cursor; // Return the original cursor if the move is invalid, otherwise update to move to the new position
    }
    /**
     * Helper method to get the state of the cell in a given direction from the current cursor position. The direction is represented as an integer (0 for up, 1 for right, 2 for down, 3 for left).
     * @param cursor
     * @param dir direction to check the state of the cell (0 for up, 1 for right, 2 for down, 3 for left)
     * @return the state character of the cell in the given direction from the current cursor position, or '#' if the new position is out of bounds
     */
    private char getStateInDirection(int[] cursor, int dir) {
        ...
        switch (dir) {
            ...
            default: System.out.println("Invalid direction: " + dir + ". Use 0 for up, 1 for right, 2 for down, 3 for left."); return '#';
        }
        return getState(newX, newY);
    }
    /**
     * Method to solve the maze using Tremaux's algorithm. The method starts at the starting position of the maze and explores the maze by prioritizing nearby cells based on their state (empty, marked once, marked twice).
     * If it reaches the end point, it prints a success message and returns true. If it encounters a cell that has been marked twice, it backtracks to the previous cell and continues exploring. 
     * If it exhausts all possible paths without reaching the end point, it prints a failure message and returns false.
     * @return true if the maze is successfully solved, false if the maze is unsolvable or if there are no valid moves available at any point during the solution process.
     */
        ...
        while (getState(cursor[0], cursor[1]) != 'E') {
            ...
            for (int dir = 0; dir < 4; dir++) {
                ...
                if (state == 'E') {
                    ...
                    bestStateValue = state;
                    bestDirection = dir;
                } else if (state == '.' && bestStateValue != ' ') {
                    bestStateValue = state;
                    bestDirection = dir;
                } else if (state == 'S' && bestStateValue != ' ') {
                    bestStateValue = state;
                    bestDirection = dir;
                }
                else if (state == ':' && bestStateValue != ' ' && bestStateValue != '.') {
                    bestStateValue = state;
                    bestDirection = dir;
                }
            }
            ...
            else if (bestStateValue == 'E') {
                cursor = moveCursor(cursor, bestDirection);
                solutionSteps++;
                backtrackStack.push(bestDirection);
                break;
            }
            else if (bestStateValue == ':' ){
                //backtrack if we are at a cell we've been to twice before, as per Tremaux's algorithm
